...
junk = fgetl(fid);
junk = fscanf(fid,'%s',1);
nin = fscanf(fid,'%d',1); %nin = πλήθος των εισόδων 
junk = fscanf(fid,'%s',1);
nout = fscanf(fid,'%d',1); %nout = πλήθος των εξόδων (3 κατηγορίες λουλουδιού) 
junk = fscanf(fid,'%s',1);
nrpat = fscanf(fid,'%d',1); %nrpat = number of patterns, πλήθος των δεδομένων 

A = fscanf(fid,'%f',[nin+nout,Inf]); %A = [I/O pairs]
fclose(fid);

x = A(1:nin,:); %Πίνακας ειδόδων (4 γραμμες * 150 στήλες )
d = A(nin+1:nin+nout,:); % Οι επιθυμιτές εξόδοι (3 γραμμές * 150 στήλες)

trpats=105; % εκπαιδειτικό σύνολο
valpats=30; % σύνολο τεκμιρίωσης
testpats=15; % σύνολο ελέγχου
                    ...
                    for ep=1:epochs % Για κάθε εποχή 
                        for iter=1:trpats % Για κάθε στοιχείο του εκπαιδευτικού συνόλου
                            ...
                            ed(c1)=min(ed); % Επανατοποθέτηση της αρχικής τιμης στην θέση c1
                               ...
                               % Συνθήκες του αλγόριθμου LVQ2
                               if  (L(c1) ~= trlabels(iter)) && (L(c2) == trlabels(iter)) && (min(d1/d2,d2/d1) > 1-s) && (max(d1/d2,d2/d1) <1+s)
                                  % Ενημέρωση του πίνακα βαρών 
